...
| 工具 | 所属智能体 | 用途 |
|---|---|---|
| `design_scene` | director | 根据场景描述生成空镜头环境设计提示词 |
| `design_prop` | director | 根据道具描述生成结构化道具设计提示词 |
| `generate_prop_three_view_sheet` | image_gen | 生成道具三视图（正面/侧面/背面）标准化设定板 |
| `generate_storyboard_grid_sheet` | image_gen | 生成多宫格分镜拼图（16/25宫格） |
| `generate_image` | image_gen | 生成普通场景图或道具单图 |
...
3. 交给 image_gen → 调用 `generate_image` 生成场景图
...
- 如果用户道具描述不足，先交给 director → 调用 `design_prop` 补全描述
