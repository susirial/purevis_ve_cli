...
| 多视图 / 表情表 / 姿势表 | 基于角色参考图生成一致性辅助素材 | 角色名、参考图路径、提示词 | 多格图、本地图片文件 | 角色统一建模、动作与表情资产准备 |
...
注：请在支持 `HTML5` 的 `Markdown` 阅读器中查看视频，或直接访问 [视频链接](https://github.com/susirial/purevis_ve_cli/blob/main/demo/%E5%B1%B1%E4%B8%8A%E5%A4%A7%E8%B1%A1%E8%BF%87%E6%B8%A1%E5%88%B0%E5%A4%A7%E8%B1%A1%E8%83%8C%E7%BE%8E%E5%A5%B3.mp4) 在线观看。
...
| `MODEL_AGENT_NAME` | 是 | 文本大模型名称，用于对话、调度、写剧本、总结等 |
| `MODEL_AGENT_API_BASE` | 否 | VeADK Agent 模型访问地址，推荐使用火山方舟 `api/v3` 端点 |
| `MODEL_AGENT_API_KEY` | 是 | 文本模型对应的 `API Key` |
...
以下示例表示“全部使用火山引擎底座”的典型配置方式。请使用您自己的密钥占位值，不要将真实 `API Key`、`AK`、`SK` 写入文档或仓库：
...
2. 如果 `MEDIA_PROVIDER=auto` 且不存在 `PUREVIS_API_KEY`，但存在 `MODEL_AGENT_API_KEY`，则回退到 `volcengine_ark`。
3. 如果显式设置了 `MEDIA_PROVIDER`，则按配置选择对应提供方。
4. 如果既没有可用的 `PUREVIS_API_KEY`，也没有可用的 `MODEL_AGENT_API_KEY`，媒体相关能力会报错并提示缺少可用服务。
...
- **为什么没有配置 `PUREVIS_API_KEY` 也可能继续生图或生视频？**  
  因为项目支持媒体提供方回退逻辑。在没有 `PUREVIS_API_KEY` 时，如果配置了 `MODEL_AGENT_API_KEY`，系统会尝试使用 `volcengine_ark`。
