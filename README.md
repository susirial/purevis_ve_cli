...
本项目的入口是 `purevis_agent.py`，其核心运行机制可以概括为以下六层：
...
本系统采用 `Multi-Agent` 架构，各司其职。所有智能体均配备 `load_skill` / `load_skill_reference` 工具，在运行时按需加载领域知识，而非将规则硬编码到 instruction 中：

1. 🧠 **Orchestrator**（21 个工具）：对接用户需求，按六大制作阶段调度各团队成员；按需加载 `project-management`、`style-configuration`、`character-design`、`media-routing` 等技能。
2. ✍️ **Director**（10 个工具）：负责题材策划、角色/场景/道具设定、剧本撰写、剧本拆解与提示词预处理；按需加载 `character-design`、`scene-prop-design` 技能。
3. 🎬 **Visual Director**（7 个工具）：负责将剧本或参考图转化为电影级关键帧序列，并输出穷尽式 `JSON` 分镜；按需加载 `storyboard-direction` 技能（含构图技法、用光术语、JSON 规范 3 个参考文档）。
4. 🎨 **Image Gen**（20 个工具）：负责文生图、图生图、多视图、表情表、姿势表及关键帧图片生成；按需加载 `image-generation` 技能。
5. 🎥 **Video Gen**（12 个工具）：负责将关键帧或参考图转化为最终视频片段；按需加载 `video-generation` 技能。
6. 🧐 **Vision Analyzer**（8 个工具）：负责视觉资产质检、审美把关和一致性分析；按需加载 `vision-analysis` 技能。
...
```text
skills/                              # 渐进式披露技能层（9 个 Skill + 12 个参考文档）
├── project-management/SKILL.md
├── style-configuration/SKILL.md
├── character-design/SKILL.md
├── scene-prop-design/SKILL.md
├── storyboard-direction/SKILL.md    # 含 3 个 references/
├── image-generation/SKILL.md
├── video-generation/SKILL.md        # 含 2 个 references/
...
<video width="720" controls preload="metadata" playsinline poster="demo/完整演示/1-11-角色参考图.png">
  <source src="https://www.bilibili.com/video/BV1gzQeBLE96/?vd_source=6f34d64f4d8fd4cc199a0fafeca5fc93" type="video/mp4">
</video>
...
2. 智能体按需调用 `load_skill(skill_name)` 获取当前任务所需的领域知识与决策规则；如需深度参考，继续调用 `load_skill_reference(skill_name, ref_file)` 加载详细文档。
3. 若涉及项目信息、风格上下文或主体库检索，先调用 `state_tools` / `style_tools`。
4. 若涉及内容设计，由 `director` 或 `visual_director` 产出文本设定、分镜拆解或 `JSON` 提示词。
5. 若涉及媒体生成，由 `image_gen` 或 `video_gen` 提交异步任务。
6. 异步任务提交后，智能体优先调用 `wait_for_task(...)` 阻塞等待，不建议在对话中频繁手动轮询。
7. 任务完成后，使用 `download_and_save_media(...)` 落盘，并在需要时调用本地预览或注册到主体库。
8. 若涉及审查，则由 `vision_analyzer` 调用 `analyze_image(...)` 获取分析结果并输出修改建议。
...
| `ZAI_API_KEY` | 否 | Z.AI API Key，仅 `zai_glm51_smoke` 测试脚本使用，主流程不需要 |
| `MEDIA_PROVIDER` | 否 | 显式指定全局默认媒体提供方（默认值兜底层），可选 `auto`、`purevis`、`libtv`、`volcengine_ark`、`vidu`、`kling`；Agent 运行时可通过工具参数动态切换 |
...
| `LITELLM_LOG` | 否 | LiteLLM 内部日志等级，推荐 `CRITICAL` 以抑制首轮冷启动超时噪声 |
...
# 图片分析工具模型（推荐固定为 doubao，不随 Agent 主模型切换）
...
# ZAI API Key（可选，仅 zai_glm51_smoke 测试脚本使用）
...
# 注：Agent 运行时可通过工具参数 model="xxx" 动态切换 provider，此处仅设定无显式指定时的默认路由。
...
# 【可选】按能力单独指定默认模型（仅在目标 provider 支持显式模型切换时生效）
