...
def get_provider_class(name: str) -> Type[BaseMediaProvider]:
...
def get_media_provider_by_name(name: str) -> BaseMediaProvider:
    return get_provider_class(name)()


def resolve_auto_provider_name() -> str:
    if os.environ.get("PUREVIS_API_KEY"):
        return "purevis"
    if os.environ.get("LIBTV_ACCESS_KEY"):
        return "libtv"
    if build_volcengine_ark_api_config().get("api_key"):
        return "volcengine_ark"
    raise FeatureUnavailableError(
        "未检测到可用的媒体生成服务：请设置 PUREVIS_API_KEY、LIBTV_ACCESS_KEY，或设置 VOLCENGINE_ARK_API_KEY。"
...
def get_media_provider() -> BaseMediaProvider:
    ...
    if provider_name == "auto":
        provider_name = resolve_auto_provider_name()
    return get_media_provider_by_name(provider_name)


from tools.media_providers import purevis_provider as _purevis_provider
from tools.media_providers import libtv_provider as _libtv_provider
